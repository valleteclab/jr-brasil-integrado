...
import { setTenantPlano, setTenantTrial, setTenantTrialAte, criarAssinaturaTenantAdmin, simularAtrasoMensalidade, definirValorMensalidadeTenant, PlatformAdminError } from "@/lib/services/platform-admin";
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (body.acao === "definir-valor-mensalidade") {
      const r = await definirValorMensalidadeTenant(params.id, body.valor ?? null);
