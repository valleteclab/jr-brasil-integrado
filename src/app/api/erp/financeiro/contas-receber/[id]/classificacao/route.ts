...
/** Define/remove a classificação financeira de uma conta a receber (null limpa). */
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    await requireModulo("financeiro");
    const scope = await getDevelopmentTenantScope();
    const body = await request.json() as { classificacaoId?: string | null };
    await setClassificacaoConta(scope, "receber", params.id, body.classificacaoId || null);
