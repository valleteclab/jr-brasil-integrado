/**
 * CA raiz ICP-Brasil (Autoridade Certificadora Raiz Brasileira v10) embutida em PEM.
 *
 * Os web services da SEFAZ usam certificados de SERVIDOR emitidos sob a ICP-Brasil, cuja raiz NÃO
 * está na store de CAs do Node (que vem com as CAs públicas/Mozilla). Sem ela, o TLS falha com
 * "unable to get local issuer certificate". O servidor da SEFAZ envia a cadeia até a intermediária;
 * confiando nesta raiz v10, a validação fecha (server → intermediária → raiz v10).
 *
 * Embutido como string (em vez de arquivo .pem) para sobreviver ao build standalone do Next, que
 * não copia arquivos avulsos. Fonte: acraiz.icpbrasil.gov.br (raiz v10, válida até 2032).
 */
export const ICP_BRASIL_ROOT_V10 = `-----BEGIN CERTIFICATE-----
