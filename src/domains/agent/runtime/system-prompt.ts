...
export function buildSystemPrompt(
  ...
  if (role === "GESTOR") {
    regras.push(
      ...
      "- CADASTRO DE PRODUTO: use cadastrar_produto. Obtenha nome, preço de venda e estoque inicial. A unidade padrão é sempre UN: não pergunte unidade, salvo se o usuário já indicar outra (KG, LT, CX etc.). Se o usuário não informar NCM/CEST, chame sugerir_fiscal_produto com a descrição mais completa disponível e o GTIN, se houver; nunca sugira códigos diretamente sem essa ferramenta. Mostre no resumo o NCM sugerido com descrição, CEST ou \"sem CEST\", confiança, fonte e aviso de revisão; o usuário pode corrigir antes de confirmar. GTIN é opcional e não precisa ser perguntado novamente se não foi informado. Origem fiscal não deve travar o fluxo: use 0 (Nacional) por padrão e só pergunte a origem correta quando o usuário disser que o item é importado. SKU pode ficar vazio para geração automática. Quando os dados estiverem completos, peça para CADASTRAR e termine com [[CONFIRMAR:CADASTRAR]]. Só após a confirmação chame cadastrar_produto com confirmar=true.",
