...
## Atualizacao operacional - 2026-06-10 - blocos G (CIAP) e K (estoque) no SPED

- BLOCO G completo: novo modelo CiapBem (migration sped_ciap_bloco_g_k) com criacao AUTOMATICA ao processar entrada com finalidade IMOBILIZADO (ICMS destacado ou credito do Simples) + cadastro manual em /erp/sped-fiscal/ciap (listar, cadastrar, baixar, excluir; auditado). Gerador emite 0300/0305 (cadastro do bem) + 0500/0600 (conta/centro de custo genericos) + G110 (saldo inicial, soma das parcelas, fator VL_TRIB_EXP/VL_TOTAL com 8 decimais, ICMS_APROP) + G125 (IM no mes da imobilizacao com G130/G140 do documento; SI nos demais) e credito mensal no E111/E110 com codigo de ajuste da tabela 5.1.1 da UF (configuravel; sem ele, gera o bloco e avisa que o credito ficou fora). Parcela = total/48 x fator de saidas tributadas (inclui ST do substituto e exportacao 7xxx); apropriacao derivada dos meses decorridos (idempotente); baixa encerra com aviso (G125 BA com o contador).
- BLOCO K (estoque): flag gerarBlocoK na configuracao gera K001=0 + K010 modo 2 (escrituracao restrita ao saldo) + K100 + K200 a partir do EstoqueSaldo agregado por produto (itens no 0200), com aviso de que o saldo informado e o ATUAL.
- UI: pagina /erp/sped-fiscal/ciap, botao CIAP na tela principal, card CIAP na apuracao (fator, parcelas por bem, escriturado ou nao), campos novos na configuracao (codAjusteCreditoCiap, bloco K).
- Estrutura validada: fixture com 2 bens (IM novo + SI antigo) e estoque K200x2 — 123 linhas, todos os contadores e larguras de campos do leiaute 020 conferidos (0300:7, 0305:4, 0500:7, 0600:4, G110:10, G125:10, G130:9, G140:9, K010:2, K100:3, K200:6); tsc e build ok.
- Blocos restantes do leiaute NAO gerados (sem dado no ERP, documentado): C101/C105 (DIFAL EC 87 consumidor final), D100/D190 (CT-e de frete tomado), 1601 (instrumentos de pagamento — cartao/PIX), B com movimento (ISS-DF). Avaliar conforme necessidade dos clientes.

## Atualizacao operacional - 2026-07-03 - formacao de precos a vista/a prazo e vinculo por codigo do fornecedor

- PRECOS: Produto ganhou preco a prazo (`precoVendaPrazo`; `precoVenda` segue sendo o A VISTA usado por PDV/vendas) e margens memorizadas (`margemVistaPercentual`/`margemPrazoPercentual`). Empresa ganhou margens padrao (`margemPadraoVistaPct`/`margemPadraoPrazoPct`, editaveis em Configuracoes) que sugerem os precos automaticamente (custo x (1 + margem/100)) no cadastro de produto e na importacao de XML. Migration `20260703160000_precos_vista_prazo` (PENDENTE de aplicar com `prisma migrate deploy`).
- CADASTRO DE PRODUTO: aba Precos com margem a vista %, preco a vista, margem a prazo %, preco a prazo; digitar a margem calcula o preco a partir do custo, mudar o custo recalcula os precos com margem definida; produto novo ja nasce com as margens padrao da empresa. Informar fornecedor + codigo no fornecedor (aba Compras) agora grava o vinculo `ProdutoFornecedor` quando o fornecedor ja existe no cadastro.
...
- VENDA (Atendimento e PDV): o vendedor escolhe o preco de cada item — tabela A VISTA (padrao), tabela A PRAZO ou preco MANUAL negociado. Atendimento: preco unitario editavel na linha + seletor de tabela; PDV: botao "R$" abre modal (a vista / a prazo / manual). Quando a forma de pagamento e a prazo (boleto/faturado/crediario), o sistema SUGERE aplicar o preco a prazo com um clique — nunca troca sozinho.
- CONTROLE COMERCIAL (server-side): novo guard `autorizarPrecosVenda` (price-guard.ts) — preco manual abaixo da tabela escolhida vira DESCONTO IMPLICITO e soma na regra `descontoSemAutorizacaoPct` (senha de admin acima do limite); preco abaixo do PRECO MINIMO do produto sempre exige senha; acima da tabela e livre. Fecha a brecha em que o servidor aceitava qualquer preco do cliente sem revalidar. Aplicado em createSale, editConfirmedSale e pdvCheckout (que agora repassa a senhaAdmin ao checkoutSale).
...
## Atualizacao operacional - 2026-07-25 - Kokoro TTS
...
## Atualizacao operacional - 2026-07-25 - estabilidade da voz no Telegram
...
## Atualizacao operacional - 2026-07-26 - IA e anexos no chat web

- Corrigida a diferenca entre o modulo de IA habilitado e a credencial efetivamente configurada: novos tenants do plano CHAT passam a receber, ainda criptografada, a configuracao OpenRouter padrao definida por `PLATFORM_AI_CONFIG_EMPRESA_ID`.
- O tenant CHAT de teste da VALLETECLAB foi corrigido sem expor a chave; chamada real ao modelo `openai/gpt-4o-mini` respondeu HTTP 200.
- O chat web agora aceita imagens JPG/PNG/WebP, PDF, TXT, CSV, XML e JSON, com limite geral de 10 MB.
...
## Atualizacao operacional - 2026-07-27 - cadastro de produtos pelo agente

- Adicionada a ferramenta `cadastrar_produto`, disponivel somente para o perfil GESTOR no chat web, WhatsApp, Telegram e MCP, pois todos usam o mesmo registro de ferramentas.
- O agente coleta nome e preco de venda, pergunta o estoque inicial quando ausente e aceita SKU opcional, tipo, marca, categoria, GTIN, unidade, descricao, NCM, CEST, origem, CFOPs, custos, precos, limites de estoque, deposito e visibilidade na loja.
- SKU vazio usa o gerador seguro `PRD-NNNNNN`; SKU duplicado e rejeitado pelo caso de uso existente e GTIN ja vinculado e bloqueado antes da criacao.
- Antes de gravar, o agente deve mostrar nome, SKU, preco, estoque, unidade e dados fiscais e pedir a resposta `CADASTRAR`; o handler recusa chamadas sem `confirmar=true`.
...
- Quando a descricao comercial nao encontra candidatos suficientes (por exemplo, `notebook`), uma etapa de expansao gera vocabulario fiscal e ate cinco codigos provaveis usados somente como chaves de recuperacao; codigos inexistentes sao descartados, os existentes viram candidatos com descricao oficial e a selecao/validacao final continua obrigatoria.
